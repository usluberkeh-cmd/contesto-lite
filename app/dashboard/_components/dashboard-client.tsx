...
type FineDisplayStatus = 'in_progress' | 'processed'

const displayStatusConfig: Record<FineDisplayStatus, { label: string; color: string }> = {
  in_progress: { label: 'En cours d’analyse', color: 'bg-blue-100 text-blue-800' },
  processed: { label: 'Traité', color: 'bg-green-100 text-green-800' },
} as const
const filterStatusOrder: FineDisplayStatus[] = ['in_progress', 'processed']
const fallbackStatus = { label: 'Statut inconnu', color: 'bg-slate-100 text-slate-700' }

function getDisplayStatus(status?: Fine['status'] | string | null): FineDisplayStatus {
  return status === 'processed' ? 'processed' : 'in_progress'
}

function getStatusConfig(status?: Fine['status'] | FineDisplayStatus | string | null) {
  const normalizedStatus = status === 'in_progress' || status === 'processed'
    ? status
    : getDisplayStatus(status)
  return displayStatusConfig[normalizedStatus] ?? fallbackStatus
}
...
function getFineDisplayNumber(fine: Fine) {
  const fineNumber = fine.fine_number?.trim()
  if (fineNumber) {
    return fineNumber
  }

  // # Reason: Show a deterministic fallback while extraction is still in progress.
  return fine.id.slice(0, 8).toUpperCase()
}

function formatFineDate(value: string | null) {
  if (!value) {
...
export default function DashboardClient({
  ...
  const handlePreviewFile = async (fine: Fine) => {
    if (!fine.file_url) {
      // Reason: We need the stored object key from file_url to create a signed URL.
  ...
  const allStatuses = filterStatusOrder
  const filteredFines =
    activeFilter === 'all'
      ? fines
      : fines.filter((fine) => getDisplayStatus(fine.status) === activeFilter)
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        ...
        <Card>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">En cours d’analyse</CardTitle>
            <AlertCircle className="h-4 w-4 text-muted-foreground" />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">
              {fines.filter((fine) => getDisplayStatus(fine.status) === 'in_progress').length}
            </div>
            <p className="text-xs text-muted-foreground">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {fines.filter((fine) => getDisplayStatus(fine.status) === 'processed').length}
            </div>
            <p className="text-xs text-muted-foreground">
              Analyses terminées
            </p>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-2">
            ...
            {allStatuses.map((status) => {
              const count = fines.filter((fine) => getDisplayStatus(fine.status) === status).length
                  ...
                  <CardContent className="p-6">
                    ...
                    <div className="space-y-3">
                      <h3 className="font-semibold text-lg text-foreground">
                        Amende #{getFineDisplayNumber(fine)}
                      </h3>
                      ...
                      <div className="space-y-2 text-sm text-muted-foreground">
                        <div className="flex items-center gap-2">
                          ...
                          <span>{fine.vehicle_plate?.trim() || 'En cours d’analyse'}</span>
                        </div>

                        <div className="flex items-center gap-2">
                          <Calendar className="w-4 h-4" />
                          <span>{formatFineDate(fine.fine_date)}</span>
                        </div>
